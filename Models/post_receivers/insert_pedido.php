...
try {
    ...
    $insertEntradaStmt->execute(array($_POST['codigo_colaborador'], date("Y-m-d h:i:sa", strtotime($dataPedido) + 1), $_POST['valor_entrada'], $caixa['caixa'], 'Entrada Pedido_' . $lastInsertedId, $_POST['pagamento'], $lastInsertedId, 1));
    ...
    foreach ($_POST['produtos'] as $key => $value) {
        ...
        $produtoStmt->execute(array($_POST['codigo_colaborador'], $dataPedido, $value['preco'], $caixa['caixa'], $value['id'], $_POST['pagamento'], $lastInsertedId, $value['quantidade']));
