...
include('../../MySql.php');
    ...
    if(!empty($codigo)){
        ...
        $sangria = \MySql::conectar()->prepare("INSERT INTO `tb_produtos` (`id`, `nome`, `preco`, `codigo`, `por_peso`, `codigo_id`, `vendido`, `ncm`, `cod_grp_financeiro`, `cst_icms`, `icms`, `cst_pis_cofins`,`json_precos`,`validade`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)");
        $sangria->execute(array($_POST['nome'],number_format($preco,2,"."),$_POST['codigo'],$_POST['por_peso'],$_POST['codigo_id'],1,$_POST["ncm"],0,$_POST["cst_icms"],$_POST["icms"],$_POST["cst_pis_cofins"],$arrayPrecos,$_POST["validade"] ));
    }
