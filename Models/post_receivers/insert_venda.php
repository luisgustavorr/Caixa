...
if(!empty($colab)){
  ...
  foreach ($_POST['produtos'] as $key => $value) {
    ...
    $produto->execute(array($_POST['colaborador'],date("Y-m-d h:i:sa"),$value['preco']*$value['quantidade'],trim($_POST['caixa']),$value['id'],$_POST['pagamento']));
    $atualizar_caixa = \MySql::conectar()->prepare("UPDATE `tb_caixas` SET `valor_atual` = `valor_atual` + ? WHERE `tb_caixas`.`caixa` = ? ");
    $atualizar_caixa->execute(array($_POST['valor'],trim($_POST['caixa'])));
